...
public class FacturaBean implements IFacturaBean {
	...
	public void save(Factura factura) {
		try {
			Query query = entityManager.createQuery("SELECT MAX(f.numero) FROM Factura f");
	...
	public Factura generateFacturaByMatricula(String matricula) {
		...
		try {
			...
			double unidades = 
				((millisSalida - millisEntrada) / Constantes.__MILISEGUNDOS_HORA)
				+ (((millisSalida - millisEntrada) % Constantes.__MILISEGUNDOS_HORA) / Constantes.__MILISEGUNDOS_MINUTO) * 0.01;
			
			facturaLinea.setUnidades(Math.max(Constantes.__MINIMO_UNIDADES_PARKING, unidades));
			
			// Cálculo de importes
			facturaLinea.setImporteUnitario(servicioPrecio.getPrecio());
			facturaLinea.setImporteTotal(facturaLinea.getImporteUnitario() * facturaLinea.getUnidades());
...
//			entityManager.persist(factura);
	...
	public void remove(Factura factura) {
		try {
			Factura managedFactura = entityManager.find(Factura.class, factura.getId());
